  ...
  init: function() {
    ...
    $.getJSON('../bikes.json', function(data) {
      ...
      for (i = 0; i < data.length; i ++) {
        petTemplate.find('.panel-title').text(data[i].location);
        petTemplate.find('img').attr('src', data[i].picture);
        petTemplate.find('.pet-breed').text(data[i].brand);
        petTemplate.find('.pet-age').text(data[i].age);
        petTemplate.find('.pet-location').text(data[i].value);
    ...
    App.contracts.Adoption.deployed().then(function(instance) {
      ...
      for (i = 0; i < adopters.length; i++) {
        if (adopters[i] !== '0x0000000000000000000000000000000000000000') {
          $('.panel-pet').eq(i).find('button').text('Purchased').attr('disabled', true).css('background-color', 'red');
        }
      }
    }).catch(function(err) {
